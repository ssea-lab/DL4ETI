...
def Network_config(class_num=4, epoch=200, initial_epoch=0, batch_size=32,
    ...
    model.load_weights('/media/myharddisk/Sun/pretrain_model/vgg16_weights_tf_dim_ordering_tf_kernels_notop.h5',
                       by_name=True)
    ...
    tools.create_directory('./retmp/')
    weights_file = './retmp/' + str(fold)+'-weights.{epoch:02d}-{categorical_accuracy:.4f}-{val_loss:.4f}-{val_categorical_accuracy:.4f}.h5'
    csv_file = './retmp/rerecord.csv'
    lr_reducer = ReduceLROnPlateau(monitor='val_categorical_accuracy', factor=0.5,
                                   cooldown=0, patience=3, min_lr=0.5e-6)
    early_stopper = EarlyStopping(monitor='val_categorical_accuracy', min_delta=1e-4, patience=10)
    ...
    print(confusion)
    f = open('re_confusion_matrix.txt','a+')
...
def main():
    ...
    cross_validation(data, K=10, epoch=100, class_num=4, batch_size=32)
main()
