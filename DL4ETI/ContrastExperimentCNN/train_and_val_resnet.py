...
def Network_config(class_num=4, epoch=200, initial_epoch=0, batch_size=32,
    ...
    base_model = ResNet50(input_tensor=Input(shape=(224, 224, 3)), weights='imagenet', include_top=False)
    ...
    tools.create_directory('./tmpresnet/')
    weights_file = './tmpresnet/' + str(fold)+'-weights.{epoch:02d}-{categorical_accuracy:.4f}-{val_loss:.4f}-{val_categorical_accuracy:.4f}.h5'
    csv_file = './tmpresnet/record.csv'
