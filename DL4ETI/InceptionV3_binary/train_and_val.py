...
def Network_config(class_num=4, epoch=200, initial_epoch=0, batch_size=32,
    ...
    base_model = InceptionV3(input_tensor=Input(shape=(224, 224, 3)), weights=None, include_top=False)
    x = base_model.output
    ...
    lr_reducer = ReduceLROnPlateau(monitor='categorical_accuracy', factor=0.5,
                                   cooldown=0, patience=8, min_lr=0.5e-6)
...
def main():
    data = creat_list('binary_random_list.txt')
