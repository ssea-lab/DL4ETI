...
def Network_config(class_num=4, epoch=200, initial_epoch=0, batch_size=32,
    ...
    tools.create_directory('./tmp2/')
    weights_file = './tmp2/' + str(fold)+'-weights.{epoch:02d}-{categorical_accuracy:.4f}-{val_loss:.4f}-{val_categorical_accuracy:.4f}.h5'
    csv_file = './tmp2/record.csv'
    lr_reducer = ReduceLROnPlateau(monitor='categorical_accuracy', factor=0.5,
                                   cooldown=0, patience=5, min_lr=0.5e-6)
    early_stopper = EarlyStopping(monitor='val_categorical_accuracy', min_delta=1e-4, patience=120)

    model_checkpoint = ModelCheckpoint(weights_file, monitor='val_categorical_accuracy', save_best_only=True,
                                       verbose=1,
                                       save_weights_only=True, mode='max')
    tensorboard = TensorBoard(log_dir='./logs2/', histogram_freq=0, batch_size=8, write_graph=True,
    ...
    print(confusion)
    f = open('confusion_matrix2.txt','a+')
...
def main():
    ...
    cross_validation(data, K=10, epoch=120, class_num=4, batch_size=32)
main()
